...
export class GitHubUpdater {
    ...
    try {
      ...
      const response = await fetch(this.apiUrl, {
        headers: {
          ...
          'User-Agent': `AGIME-Desktop/${app.getVersion()}`,
        },
      });
      ...
      if (platform === 'darwin') {
        ...
        if (arch === 'arm64') {
          ...
          assetName = 'AGIME_intel_mac.zip';
        }
      } else if (platform === 'win32') {
        // Windows - for future support
        assetName = 'AGIME-win32-x64.zip';
      } else {
        // Linux - for future support
        assetName = `AGIME-linux-${arch}.zip`;
      }
  ...
  async downloadUpdate(
    ...
    try {
      ...
      const fileName = `agime-${latestVersion}.zip`;
