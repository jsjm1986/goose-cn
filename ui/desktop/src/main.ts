...
if (MAIN_WINDOW_VITE_DEV_SERVER_URL) {
  ...
  console.log('[Main] Development mode: Forcing protocol registration for agime://');
  app.setAsDefaultProtocolClient('agime');

  if (process.platform === 'darwin') {
    try {
      // Reset the default handler to ensure dev version takes precedence
      spawn('open', ['-a', process.execPath, '--args', '--reset-protocol-handler', 'agime'], {
        detached: true,
        stdio: 'ignore',
      });
    } catch {
...
if (process.platform !== 'darwin') {
  ...
  if (!gotTheLock) {
    ...
    app.on('second-instance', (_event, commandLine) => {
      const protocolUrl = commandLine.find((arg) => arg.startsWith('agime://'));
      if (protocolUrl) {
  ...
  const protocolUrl = process.argv.find((arg) => arg.startsWith('agime://'));
  if (protocolUrl) {
    app.whenReady().then(() => {
      handleProtocolUrl(protocolUrl);
    });
...
// Track chat busy state per window (true = AI is processing)
const windowChatBusyState = new Map<number, boolean>(); // windowId -> isBusy
...
const createChat = async (
  ...
  mainWindow.on('close', (event) => {
    const isBusy = windowChatBusyState.get(windowId) || false;
    if (isBusy) {
      event.preventDefault();
      dialog
        .showMessageBox(mainWindow, {
          ...
          cancelId: 0,
          title: 'AGIME 正在运行',
          message: 'AI 正在处理任务，关闭窗口将终止当前工作。',
          detail: '确定要关闭吗？未完成的工作可能会丢失。',
        })
        .then((result) => {
          if (result.response === 1) {
            // User chose to force close
            windowChatBusyState.delete(windowId);
            mainWindow.destroy();
          }
        });
    }
...
ipcMain.handle('open-notifications-settings', async () => {
  try {
    if (process.platform === 'darwin') {
      ...
      const commands = [
        { cmd: 'gnome-control-center', args: ['notifications'], name: 'GNOME' },
        { cmd: 'systemsettings5', args: ['kcm_notifications'], name: 'KDE' },
        { cmd: 'xfce4-settings-manager', args: [], name: 'XFCE' },
        { cmd: 'gnome-control-center', args: [], name: 'GNOME (general)' },
      ];

      for (const { cmd, args, name } of commands) {
        const success = await trySpawnCommand(cmd, args);
        if (success) {
          console.log(`Opened notification settings using ${name}`);
          return true;
        }
        console.log(`${name} settings not available, trying next...`);
      }
    ...
    return new Promise((resolve) => {
      if (process.platform === 'win32') {
        ...
        tasklist.on('close', (code) => {
          if (code !== 0) {
            console.error('Error executing tasklist command');
            return resolve(false);
          }

          // tasklist returns "INFO: No tasks are running..." if not found
          const isRunning = output.toLowerCase().includes('ollama.exe');
          console.log('Ollama check (Windows):', isRunning ? 'running' : 'not running');
          resolve(isRunning);
        });

        tasklist.on('error', (error) => {
          console.error('Error executing tasklist command:', error);
          resolve(false);
        });
      } else {
        ...
        grep.on('close', (code) => {
          ...
          console.log('Ollama check (Unix):', isRunning ? 'running' : 'not running');
          resolve(isRunning);
        });
...
async function appMain() {
  ...
  // App menu
  const appMenu = menu?.items.find((item) => item.label === 'AGIME');
  if (appMenu?.submenu) {
  ...
  if (menu && process.platform !== 'darwin') {
    ...
    if (helpMenu.submenu) {
      ...
      // Create the About AGIME menu item with a submenu
      const aboutAgimeMenuItem = new MenuItem({
        label: 'About AGIME',
        submenu: Menu.buildFromTemplate([]), // Start with an empty submenu for About
      });

      // Add the Version menu item (display only) to the About AGIME submenu
      if (aboutAgimeMenuItem.submenu) {
        aboutAgimeMenuItem.submenu.append(
          new MenuItem({
      ...
      helpMenu.submenu.append(aboutAgimeMenuItem);
    }
  }

  // Menu translations for Chinese localization
  ...
  // Handle chat busy state updates from renderer
  ipcMain.on('set-chat-busy', (event, busy: boolean) => {
    const window = BrowserWindow.fromWebContents(event.sender);
    if (window) {
      windowChatBusyState.set(window.id, busy);
    }
  });
  ...
  ipcMain.handle('fetch-metadata', async (_event, url) => {
    try {
      ...
      const response = await fetch(url, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; AGIME/1.0)',
        },
      });
    ...
    try {
      ...
      if (process.platform === 'darwin') {
        ...
        for (const binary of chromeBinaries) {
          const success = await trySpawnCommand(binary, [url]);
          if (success) {
            console.log(`Opened URL in ${binary}`);
            opened = true;
            break;
          }
        }
...
app.whenReady().then(async () => {
  try {
    ...
    dialog.showErrorBox('AGIME Error', `Failed to create main window: ${error}`);
    app.quit();
  }
});
