...
import { getSubtitle, getFriendlyTitle } from '../extensions/subcomponents/ExtensionList';

function RuleItem({ title, extensionName, description }: { title: string; extensionName: string; description: string }) {
...
export default function PermissionRulesModal({ isOpen, onClose }: PermissionRulesModalProps) {
  ...
  const { t: tExtensions } = useTranslation('extensions');
      ...
      <DialogContent className="sm:max-w-[800px] max-h-[80vh] p-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-8 pb-8 min-h-0">
          <div className="space-y-4">
            ...
            <RulesSection
              title={t('permission.extensionRules')}
              rules={
                <div className="space-y-2">
                  {extensions.map((extension) => {
                    const subtitle = getSubtitle(extension, tExtensions);
                    return (
                      <RuleItem
                        key={extension.name}
