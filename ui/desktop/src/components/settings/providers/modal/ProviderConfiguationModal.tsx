...
export default function ProviderConfigurationModal({
  ...
  const headerText = showDeleteConfirmation
    ? t('provider.deleteConfigFor', { provider: provider.metadata.display_name })
    : t('provider.configureProvider', { provider: provider.metadata.display_name });

  const descriptionText = showDeleteConfirmation
    ? isActiveProvider
      ? t('provider.cannotDeleteActiveDescription')
      : t('provider.permanentDeleteDescription')
    : t('provider.addApiKeyDescription');
  ...
  const handleSubmitForm = async (e: React.FormEvent) => {
    ...
    parameters.forEach((parameter) => {
      if (
        ...
        errors[parameter.name] = t('provider.parameterRequired', { parameter: parameter.name });
      }
    });
    ...
    <>
      <Dialog open={!!error} onOpenChange={(open) => !open && setError(null)}>
        <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
          <DialogTitle className="flex items-center gap-2">{t('provider.error')}</DialogTitle>
          <DialogDescription className="text-inherit text-base">
            {t('provider.configCheckError')}
          </DialogDescription>
          <pre className="ml-2">{error}</pre>
          <div>{t('provider.configCheckAgain')}</div>
          <DialogFooter>
