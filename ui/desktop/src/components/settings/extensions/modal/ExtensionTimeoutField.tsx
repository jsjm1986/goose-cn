    ...
    <div className="flex flex-col gap-4 mb-6">
      ...
      <div className="flex flex-col">
        <div className="flex-1">
          <label className="text-sm font-medium mb-2 block text-textStandard">{t('form.timeout')}</label>
        </div>
        ...
        {submitAttempted && !isTimeoutValid() && (
          <div className="absolute text-xs text-red-500 mt-1">{t('form.timeoutRequired')}</div>
        )}
      </div>
    </div>
  );
