...
export const InterruptionHandler: React.FC<InterruptionHandlerProps> = ({
  ...
  const getActionTitle = () => {
    switch (match.keyword.action) {
      case 'stop':
        return t('interruption.stop');
      case 'pause':
        return t('interruption.pause');
      case 'redirect':
        return t('interruption.redirect');
      default:
        return t('interruption.interrupt');
    }
  };
  ...
  const getActionDescription = () => {
    switch (match.keyword.action) {
      case 'stop':
        return t('interruption.stopDesc');
      case 'pause':
        return t('interruption.pauseDesc');
      case 'redirect':
        return t('interruption.redirectDesc');
      default:
        return t('interruption.interruptDesc');
    }
  };
        ...
        >
          ...
          <div className="p-6 border-b border-current/10">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className={`text-sm mt-1 ${colors.accent}`}>{t('interruption.detected', { text: match.matchedText })}</p>
              </div>
              <div
                className={`text-xs px-2 py-1 rounded-full bg-white/30 dark:bg-black/20 ${colors.text}`}
              >
                {t('interruption.confident', { percent: Math.round(match.confidence * 100) })}
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            {showRedirectInput && (
              <div className="mb-4 space-y-2">
                <label className={`text-sm font-medium ${colors.text}`}>
                  {t('interruption.newTaskLabel')}
                </label>
                <textarea
                  value={redirectMessage}
                  onChange={(e) => setRedirectMessage(e.target.value)}
                  placeholder={t('interruption.newTaskPlaceholder')}
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between text-xs mb-1">
                <span className={colors.accent}>{t('interruption.detectionConfidence')}</span>
          ...
          <div className="p-6 pt-0 flex gap-3">
            ...
            >
              ...
              {showRedirectInput
                ? t('interruption.redirectBtn')
                : match.keyword.action === 'stop'
                  ? t('interruption.stopBtn')
                  : t('interruption.confirmBtn')}
            </Button>
          </div>
        </div>
