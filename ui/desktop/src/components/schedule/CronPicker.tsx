  ...
  useEffect(() => {
    ...
    if (cron) {
      ...
      try {
        ...
        isValid(false);
        setReadableCron(t('cronPicker.error', { message: errorMessage(e) }));
      }
    }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <span className="text-sm font-medium">{t('cronPicker.every')}</span>
        <select
          value={period}
          onChange={(e) => setPeriod(e.target.value as Period)}
          className={selectClassName}
        >
          <option value="minute">{t('cronPicker.minute')}</option>
          <option value="hour">{t('cronPicker.hour')}</option>
          <option value="day">{t('cronPicker.day')}</option>
          <option value="week">{t('cronPicker.week')}</option>
          <option value="month">{t('cronPicker.month')}</option>
          <option value="year">{t('cronPicker.year')}</option>
        </select>
      </div>
      ...
      <div className="space-y-3">
        {period === 'year' && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.in')}</span>
            <select
              value={month}
              onChange={(e) => setMonth(e.target.value)}
              className={selectClassName}
            >
              <option value="1">{t('cronPicker.january')}</option>
              <option value="2">{t('cronPicker.february')}</option>
              <option value="3">{t('cronPicker.march')}</option>
              <option value="4">{t('cronPicker.april')}</option>
              <option value="5">{t('cronPicker.may')}</option>
              <option value="6">{t('cronPicker.june')}</option>
              <option value="7">{t('cronPicker.july')}</option>
              <option value="8">{t('cronPicker.august')}</option>
              <option value="9">{t('cronPicker.september')}</option>
              <option value="10">{t('cronPicker.october')}</option>
              <option value="11">{t('cronPicker.november')}</option>
              <option value="12">{t('cronPicker.december')}</option>
            </select>
          </div>
        )}
        ...
        {(period === 'month' || period === 'year') && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.onDay')}</span>
            <input
              type="number"
              min="1"
              max="31"
        ...
        {period === 'week' && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.on')}</span>
            <select
              value={dayOfWeek}
              onChange={(e) => setDayOfWeek(e.target.value)}
              className={selectClassName}
            >
              <option value="0">{t('cronPicker.sunday')}</option>
              <option value="1">{t('cronPicker.monday')}</option>
              <option value="2">{t('cronPicker.tuesday')}</option>
              <option value="3">{t('cronPicker.wednesday')}</option>
              <option value="4">{t('cronPicker.thursday')}</option>
              <option value="5">{t('cronPicker.friday')}</option>
              <option value="6">{t('cronPicker.saturday')}</option>
            </select>
          </div>
        )}
        ...
        {(period === 'day' || period === 'week' || period === 'month' || period === 'year') && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.at')}</span>
            <input
              type="number"
              min="1"
              max="12"
            ...
            <select
              ...
              className={selectClassName}
            >
              <option value="AM">{t('cronPicker.am')}</option>
              <option value="PM">{t('cronPicker.pm')}</option>
            </select>
          </div>
        )}
        ...
        {period === 'hour' && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.atMinute')}</span>
            <input
              type="number"
              min="0"
              max="59"
        ...
        {period === 'minute' && (
          <div className="flex items-center gap-2">
            <span className="text-sm">{t('cronPicker.atSecond')}</span>
            <input
              type="number"
              min="0"
              max="59"
