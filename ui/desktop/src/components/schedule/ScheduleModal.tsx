...
export const ScheduleModal: React.FC<ScheduleModalProps> = ({
    ...
    if (value.trim()) {
      try {
        ...
        if (recipe) {
          ...
          setInternalValidationError(t('modal.invalidDeeplink'));
        }
      } catch {
        setParsedRecipe(null);
        setInternalValidationError(t('modal.parseDeeplinkFailed'));
      }
    } else {
      setParsedRecipe(null);
    }
  ...
  const handleBrowseFile = async () => {
    ...
    if (filePath) {
      if (filePath.endsWith('.yaml') || filePath.endsWith('.yml')) {
        ...
        setInternalValidationError(t('modal.invalidFileType'));
      }
    }
  };
  ...
  const handleLocalSubmit = async (event: FormEvent) => {
    ...
    if (!scheduleId.trim()) {
      setInternalValidationError(t('modal.idRequired'));
      return;
    }
    ...
    if (sourceType === 'file') {
      if (!recipeSourcePath) {
        setInternalValidationError(t('modal.sourceRequired'));
        return;
      }
      finalRecipeSource = recipeSourcePath;
    } else if (sourceType === 'deeplink') {
      if (!deepLinkInput.trim()) {
        setInternalValidationError(t('modal.deeplinkRequired'));
        return;
      }
      if (!parsedRecipe) {
        setInternalValidationError(t('modal.invalidDeeplinkFormat'));
        return;
      }
      ...
      try {
        ...
        if (!writeSuccess) {
          setInternalValidationError(t('modal.tempFileFailed'));
          return;
        }
        ...
        setInternalValidationError(t('modal.processRecipeFailed'));
        return;
      }
    }
        ...
        >
          ...
          {!isEditMode && (
            <>
              ...
              <div>
                <label className={modalLabelClassName}>{t('modal.sourceLabel')}</label>
                    ...
                    <div>
                      ...
                      {recipeSourcePath && (
                        <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 italic">
                          {t('modal.selected', { path: recipeSourcePath })}
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  {sourceType === 'deeplink' && (
                    <div>
                      <Input
                        ...
                        placeholder={t('modal.deeplinkPlaceholder')}
                        className="rounded-full"
                      />
                      {parsedRecipe && (
                        <div className="mt-2 p-2 bg-green-100 dark:bg-green-900/30 rounded-md border border-green-500/50">
                          <p className="text-xs text-green-700 dark:text-green-300 font-medium">
                            {t('modal.parsedSuccess')}
                          </p>
                          <p className="text-xs text-green-600 dark:text-green-400">
                            {t('modal.parsedTitle', { title: parsedRecipe.title })}
                          </p>
                          <p className="text-xs text-green-600 dark:text-green-400">
                            {t('modal.parsedDescription', { description: parsedRecipe.description })}
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div>
            <label className={modalLabelClassName}>{t('modal.scheduleLabel')}</label>
          ...
          >
            {isLoadingExternally
              ? isEditMode
                ...
                : t('modal.creating')
              : isEditMode
                ? t('modal.updateSchedule')
                : t('modal.createSchedule')}
          </Button>
        </div>
      </Card>
    </div>
