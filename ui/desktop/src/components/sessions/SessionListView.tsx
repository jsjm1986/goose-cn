  ...
  ({ session, isOpen, onClose, onSave, disabled = false }) => {
    ...
    const handleSave = useCallback(async () => {
      ...
      try {
        ...
        toast.error(`${t('descriptionUpdateFailed')}: ${errorMessage}`);
        setDescription(session.name);
      } finally {
        setIsUpdating(false);
      }
    ...
    return (
      <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50">
        <div className="bg-background-default border border-border-subtle rounded-lg p-6 w-[500px] max-w-[90vw]">
          <h3 className="text-lg font-medium text-text-standard mb-4">{t('editDescription')}</h3>
...
const SessionListView: React.FC<SessionListViewProps> = React.memo(
  ({ onSelectSession, selectedSessionId, onSessionDeleted }) => {
    const { t } = useTranslation('sessions');
    const { t: tCommon } = useTranslation('common');
    ...
    const handleConfirmDelete = useCallback(async () => {
      ...
      try {
        ...
        onSessionDeleted?.(sessionToDeleteId);
      } catch (error) {
    ...
    const handleImportSession = useCallback(
      async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
          ...
          toast.error(`${t('sessionImportFailed')}: ${error}`);
        } finally {
      ...
      if (sessions.length === 0) {
        return (
          <div className="flex flex-col justify-center h-full text-text-muted">
            ...
            <p className="text-lg mb-2">{t('noSessions')}</p>
      ...
      if (dateGroups.length === 0 && searchResults !== null) {
        return (
          <div className="flex flex-col items-center justify-center h-full text-text-muted mt-4">
            <MessageSquareText className="h-12 w-12 mb-4" />
            <p className="text-lg mb-2">{t('noMatchingSessions')}</p>
    ...
    return (
      <>
        ...
        <ConfirmationModal
          ...
          message={`${t('deleteConfirm', { name: sessionToDelete?.name })} ${t('deleteWarning')}`}
