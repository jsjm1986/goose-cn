  ...
  const handleSave = useCallback(
    (editType: 'fork' | 'edit' = 'fork') => {
      if (editContent.trim().length === 0) {
        setError(t('userMessage.emptyMessageError'));
        return;
      }
        ...
        {isEditing ? (
          ...
          <div className="w-full max-w-4xl mx-auto bg-background-light dark:bg-background-dark text-text-prominent rounded-xl border border-border-subtle shadow-lg py-4 px-4 my-2 transition-all duration-200 ease-in-out">
            <textarea
              ...
              placeholder={t('userMessage.editPlaceholder')}
              aria-label={t('userMessage.editLabel')}
            ...
            <div className="flex justify-between items-center mt-4">
              <div className="text-xs text-text-subtle">
                <span className="font-semibold">{t('userMessage.editInPlaceHint')}</span> {t('userMessage.editInPlaceDescription')} •{' '}
                <span className="font-semibold">{t('userMessage.forkSessionHint')}</span> {t('userMessage.forkSessionDescription')}
              </div>
              <div className="flex gap-3">
                <Button onClick={handleCancel} variant="ghost" aria-label={t('userMessage.cancelEditing')}>
                  {t('common:cancel')}
                </Button>
                <Button
                  onClick={() => handleSave('edit')}
                  variant="secondary"
                  aria-label={t('userMessage.editInPlaceLabel')}
                  title={t('userMessage.editInPlaceTitle')}
                >
                  {t('userMessage.editInPlace')}
                </Button>
                <Button
                  onClick={() => handleSave('fork')}
                  aria-label={t('userMessage.forkSessionLabel')}
                  title={t('userMessage.forkSessionTitle')}
                >
                  {t('userMessage.forkSession')}
                </Button>
              </div>
            </div>
          </div>
                ...
                <div className="relative h-[22px] flex justify-end text-right">
                  ...
                  <div className="absolute right-0 pt-1 flex items-center gap-2">
                    <button
                      ...
                      title={t('userMessage.editMessage')}
                    >
                      <Edit className="h-3 w-3" />
                      <span>{t('userMessage.edit')}</span>
                    </button>
