...
const ToastComponent = ({
  ...
  const title = t('mcpui.messageTitle', { type: messageType });
  ...
  return (
    <div className="flex flex-col gap-0 py-2 pr-4">
      ...
      {isImplemented ? (
        <p>
          {t('mcpui.messageReceived', { name: message })}
        </p>
      ) : (
        <p>
          {t('mcpui.messageReceived', { name: message })}
          <br />
          {t('mcpui.notSupported', { type: messageType })}
        </p>
      )}
    </div>
  );
...
export default function MCPUIResourceRenderer({
  ...
  const handleUIAction = async (actionEvent: UIActionResult): Promise<UIActionHandlerResult> => {
    ...
    const handleToolAction = async (
      ...
      toast.info(<ToastComponent messageType="tool" message={toolName} isImplemented={false} t={t} />, {
    ...
    const handleNotifyAction = async (
      ...
      toast.info(<ToastComponent messageType="notify" message={message} isImplemented={true} t={t} />, {
        theme: currentThemeValue,
      });
      return {
        status: 'success' as const,
