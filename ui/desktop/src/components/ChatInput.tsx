...
export default function ChatInput({
  ...
  const {
    ...
    onError: (error) => {
      toastError({
        title: t('dictation.error'),
        msg: error.message,
      });
    },
    onSizeWarning: (sizeMB) => {
      toastError({
        title: t('dictation.recordingSizeWarning'),
        msg: t('dictation.recordingSizeMessage', { size: sizeMB.toFixed(1) }),
      });
    },
  });
  ...
  const handleRetryImageSave = async (imageId: string) => {
    ...
    try {
      ...
      setPastedImages((prev) =>
        prev.map((img) =>
          img.id === imageId
            ? { ...img, error: t('images.failedToSave'), isLoading: false }
            : img
        )
      );
    }
  ...
  useEffect(() => {
    ...
    if (toolCount !== null && toolCount > TOOLS_MAX_SUGGESTED) {
      addAlert({
        ...
        message: t('tooManyTools', { count: toolCount, recommended: TOOLS_MAX_SUGGESTED }),
        action: {
          text: t('viewExtensions'),
                ...
                <Tooltip>
                  ...
                  <TooltipContent>
                    {dictationSettings.provider === 'openai' ? (
                      <p>{t('dictation.openaiNotConfigured')}</p>
                    ) : VOICE_DICTATION_ELEVENLABS_ENABLED &&
                      dictationSettings.provider === 'elevenlabs' ? (
                      <p>{t('dictation.elevenlabsNotConfigured')}</p>
                    ) : dictationSettings.provider === null ? (
                      <p>{t('dictation.notConfigured')}</p>
                    ) : (
                      <p>{t('dictation.providerNotConfigured')}</p>
                    )}
                  </TooltipContent>
                </Tooltip>
              ) : (
            ...
            <div className="absolute right-0 -top-8 bg-background-default px-2 py-1 rounded text-xs whitespace-nowrap shadow-md border border-borderSubtle">
              {isTranscribing ? (
                ...
                >
                  ...
                  {estimatedSize > 20 && <span className="text-xs">{t('recordingNearLimit')}</span>}
                </span>
              )}
            </div>
          )}
              ...
              {file.isImage ? (
                ...
                <div className="flex items-center gap-2 px-3 py-2 bg-bgSubtle border border-borderStandard rounded-lg min-w-[120px] max-w-[200px]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-textSubtle">{file.type || t('files.unknownType')}</p>
                  </div>
                </div>
              )}
              {!file.isLoading && (
            ...
            <>
              ...
              <div className="flex items-center h-full">
                <Tooltip>
                  ...
                  <TooltipContent>
                    {recipe ? t('viewEditRecipe') : t('createRecipe')}
                  </TooltipContent>
                </Tooltip>
              </div>
            </>
