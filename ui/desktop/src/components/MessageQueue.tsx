...
export const MessageQueue: React.FC<MessageQueueProps> = ({
  ...
  const formatTimestamp = (timestamp: number) => {
    ...
    if (diff < 60000) return t('queue.now');
          ...
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-foreground">
                {isPaused ? t('queue.paused') : t('queue.next')}
              </span>
            </div>
    ...
    <div className={`relative ${className}`}>
      ...
      <div className="flex items-center justify-between px-4 py-3 bg-background border-b border-border/30">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-sm font-medium text-foreground">
              {isPaused ? t('queue.paused') : t('queue.title')}
            </span>
            <span className="text-xs text-muted-foreground">
              {t('queue.messageCount', { count: queuedMessages.length })}
              {isPaused ? ` ${t('queue.waiting')}` : ` ${t('queue.queued')}`}
            </span>
          </div>
        </div>
              ...
              <div className="flex-1 min-w-0">
                {editingMessage === message.id ? (
                  ...
                  <p
                    ...
                    title={`${message.content} ${t('queue.clickToEdit')}`}
                    onClick={() => {
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                {onStopAndSend && (
                  <Button
                    ...
                    title={
                      editingMessage === message.id
                        ? t('queue.cannotSendWhileEditing')
                        : t('queue.stopAndSend')
                    }
                  >
                    <Send className="w-3 h-3" />
      ...
      {onReorderMessages && queuedMessages.length > 1 && (
        <div className="px-4 pb-3 text-xs text-muted-foreground flex items-center gap-2 opacity-60 hover:opacity-100 transition-opacity">
          ...
          <span>{t('queue.dragToReorder')}</span>
        </div>
      )}
    </div>
  );
