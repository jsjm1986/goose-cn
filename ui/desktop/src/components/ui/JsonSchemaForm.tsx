...
export default function JsonSchemaForm({
    ...
    (key: string, value: unknown): string | null => {
      ...
      if (prop.type === 'string' && typeof value === 'string') {
        ...
        if (prop.minLength !== undefined && value.length < prop.minLength) {
          return t('form.minLength', { length: prop.minLength });
        }
        if (prop.maxLength !== undefined && value.length > prop.maxLength) {
          return t('form.maxLength', { length: prop.maxLength });
        }
      }

      if ((prop.type === 'number' || prop.type === 'integer') && typeof value === 'number') {
        if (prop.minimum !== undefined && value < prop.minimum) {
          return t('form.minValue', { value: prop.minimum });
        }
        if (prop.maximum !== undefined && value > prop.maximum) {
          return t('form.maxValue', { value: prop.maximum });
        }
      }
  ...
  const renderField = (key: string, prop: JsonSchemaProperty) => {
    ...
    if (prop.enum) {
      return (
        ...
        >
          {!isRequired && <option value="">{t('form.select')}</option>}
  ...
  if (!schema.properties || Object.keys(schema.properties).length === 0) {
    return <div className="text-textSubtle text-sm">{t('form.noFields')}</div>;
  }
