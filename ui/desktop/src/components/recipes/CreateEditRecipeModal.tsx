...
export default function CreateEditRecipeModal({
  ...
  const handleSaveRecipeClick = async () => {
    if (!validateForm()) {
      toastError({
        title: tCommon('validationFailed'),
        msg: t('createEditModal.fillRequiredFields'),
      });
      return;
    }
    ...
    try {
      ...
      toastSuccess({
        ...
        msg: t('createEditModal.recipeSaved'),
      });
    } catch (error) {
      ...
      toastError({
        ...
        msg: t('createEditModal.failedToSaveRecipe', { error: error instanceof Error ? error.message : 'Unknown error' }),
  ...
  const handleSaveAndRunRecipeClick = async () => {
    ...
    try {
      ...
      toastSuccess({
        title: recipe.title,
        msg: t('createEditModal.recipeSavedAndLaunched'),
      });
    } catch (error) {
      console.error('Failed to save and run recipe:', error);

      toastError({
        title: tCommon('saveAndRunFailed'),
        msg: t('createEditModal.failedToSaveAndRunRecipe', { error: error instanceof Error ? error.message : 'Unknown error' }),
        traceback: error instanceof Error ? error.message : String(error),
      });
    } finally {
      setIsSaving(false);
            ...
            <div className="w-full p-4 bg-bgSubtle rounded-lg mt-6">
              <div className="flex items-center justify-between mb-2">
                <div className="text-sm text-textSubtle">
                  {t('createEditModal.deeplinkHint')}
                </div>
                <Button
                  onClick={handleCopy}
                  variant="ghost"
              ...
              >
                {isGeneratingDeeplink
                  ? t('createEditModal.generatingDeeplink')
                  : deeplink || t('createEditModal.clickToGenerate')}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-borderSubtle">
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              {isSaving ? tCommon('saving') : t('createEditModal.saveRecipe')}
            </Button>
            <Button
            ...
            >
              ...
              {isSaving ? tCommon('saving') : t('createEditModal.saveAndRunRecipe')}
            </Button>
          </div>
        </div>
      </div>
