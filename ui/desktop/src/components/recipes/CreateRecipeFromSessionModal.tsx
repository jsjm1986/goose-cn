...
export default function CreateRecipeFromSessionModal({
  ...
  useEffect(() => {
    if (isOpen && sessionId && !hasAnalyzed) {
      ...
      const stages = [
        t('createFromSession.stages.reading'),
        t('createFromSession.stages.identifying'),
        t('createFromSession.stages.extracting'),
        t('createFromSession.stages.generating'),
        t('createFromSession.stages.finalizing'),
      ];
      ...
      createRecipe({
        ...
        .then((response) => {
          ...
          setAnalysisStage(t('createFromSession.complete'));
        ...
        .catch((error) => {
          ...
          setAnalysisStage(t('createFromSession.analysisFailed'));
        })
        .finally(() => {
          clearInterval(stageInterval);
          setHasAnalyzed(true);
  ...
  const handleCreateRecipe = async (formData: RecipeFormData, runAfterSave = false) => {
    ...
    try {
      ...
      toastError({
        title: t('createFromSession.failedToCreate'),
        msg:
          error instanceof Error
            ? error.message
            : t('createFromSession.unexpectedError'),
      });
    } finally {
      setIsCreating(false);
    }
            ...
            {!isAnalyzing && (
              <>
                ...
                >
                  ...
                  {isCreating ? t('createFromSession.creating') : t('createFromSession.createRecipe')}
                </Button>
                <Button
                  onClick={() => {
                ...
                >
                  ...
                  {isCreating ? t('createFromSession.creating') : t('createFromSession.createAndRun')}
                </Button>
              </>
            )}
          </div>
