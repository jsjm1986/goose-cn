 ...
 * @param url The deep link URL (agime://sessions/:shareToken)
...
export async function openSharedSessionFromDeepLink(
  ...
  try {
    if (!url.startsWith('agime://sessions/')) {
      throw new Error('Invalid URL: URL must use the agime://sessions/ scheme');
    }

    // Extract the share token from the URL
    const shareToken: string = url.replace('agime://sessions/', '');
    ...
    setView('sharedSession', {
      ...
      shareToken: url.replace('agime://sessions/', ''),
      baseUrl,
    });
